let DOMEvents = (function() {
	...
	let eventsMap = {
		onclick: {
			'handle-click-workspace-tab': (evt) => uiWorkspace.OnClickWorkspaceTab(evt.target),
		},
		onmousedown: {
			'handle-mouse-workspace-tab': (evt) => uiWorkspace.HandleMouseEvt(evt),
		},
		clickable: {
	...
	function Init() {
		window.listenOn('.clickable', 'click', eventsMap.clickable);
		window.listenOn('.dom-input', 'input', eventsMap.inputCallbacks);
		window.listenOn('.submittable', 'submit', eventsMap.submittable);
		window.listening('[data-onchange]', 'onchange', 'change', eventsMap.onchange);
