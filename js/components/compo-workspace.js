let compoWorkspace = (function() {
  ...
  function GetAll() {
    return window.appSettings.data.workspaces;
  }
...
let uxWorkspace = (function() {
  ...
  let actionEvents = {
    workspaceItem: {
      'edit': (data) => showWorkspaceEditModal(data),
  ...
  async function showWorkspaceEditModal(data) {
    let workspace = getWorkspaceById(data.id);
    if (!workspace) return;
    ...
    let formStateClass = 'edit';
    let modalFormSelector = '#modal-form-workspace';
    let formResult = await window.ui.ShowModalForm(modalFormSelector, formData, formStateClass, formCallback);
    
    // console.log(formResult)
    
    // let title = window.prompt('Rename workspace', workspace.name);
  ...
  function ListWorkspacesTab() {
    $('#list-workspace-tab').innerHTML = '';

    let docFrag = document.createDocumentFragment();
    let labelUID = performance.now();
    let index = 0;
    for (let workspaceId of window.appSettings.data.workspacesTab) {
